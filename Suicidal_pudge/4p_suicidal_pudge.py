...
class Pudge(pygame.sprite.Sprite):
	...
	def update(self):
		abcdlist = [player1, player2, player3, player4]
		alist = [player1, player2, player3]
		blist = [player1, player2, player4]
		clist = [player2, player3, player4]
		dlist = [player1, player3, player4]
		elist = [player1, player2]
		flist = [player1, player3]
		glist = [player1, player4]
		hlist = [player2, player3]
		ilist = [player2, player4]
		jlist = [player3, player4]
		...
		cua = (((self.rect.centery - player4.rect.centery)**2 + (self.rect.centerx - player4.rect.centerx)**2)**(1/2))
		self.hp -= 0.3
		if self.hp < 0:
			self.hp = 0
		if self.hp == 0:
		...
		if player1.hp > 0 and player2.hp > 0 and player3.hp > 0 and player4.hp > 0:
			...
			if uno < dos and uno < tre and uno < cua:
				self.target = player1
		
			if uno > dos and tre > dos and cua > dos:
				self.target = player2
		
			if tre < dos and tre < uno and tre < cua:
				self.target = player3

			if cua < dos and cua < tre and cua < uno:
				self.target = player4
					
			if uno  == dos == tre == cua:
				self.target = random.choice(abcdlist)
			if uno > tre and dos > tre and tre == cua:
				self.target = random.choice(jlist)
			if dos > uno and tre > uno and uno == cua:
				self.target = random.choice(glist)
			if uno > dos and tre > dos and dos == cua:
				self.target = random.choice(ilist)
			if cua > uno and dos > uno and tre == uno:
				target = random.choice(flist)
			if cua > dos and uno > dos and dos == tre:
				self.target = random.choice(hlist)
			if cua > tre and uno > tre and dos == uno:
				self.target = random.choice(elist)
		...
		if player1.hp > 0 and player2.hp > 0 and player3.hp > 0 and player4.hp <= 0:
			...
			if uno > dos and dos == tre:
				self.target = random.choice(hlist)
			if dos > uno and uno == tre:
				self.target = random.choice(flist)
			if tre > uno and uno == dos:
				self.target = random.choice(elist)
		...
		if player1.hp > 0 and player2.hp > 0 and player3.hp <= 0 and player4.hp > 0:
			...
			if uno < dos and uno < cua:
				self.target = player1
		
			if uno > dos and cua > dos:
				self.target = player2

			if cua < dos and cua < uno:
				self.target = player4		
					
			if uno == dos == cua:
				self.target = random.choice(blist)
			if uno > dos and dos == cua:
				self.target = random.choice(ilist)
			if dos > uno and uno == cua:
				self.target = random.choice(glist)
			if cua > uno and uno == dos:
				self.target = random.choice(elist)
		...
		if player1.hp > 0 and player2.hp <= 0 and player3.hp > 0 and player4.hp > 0:
			...
			if uno < tre and uno < cua:
				self.target = player1
		
			if uno > tre and cua > tre:
				self.target = player3

			if cua < tre and cua < uno:
				self.target = player4		
					
			if uno == tre == cua:
				self.target = random.choice(dlist)
			if uno > tre and tre == cua:
				self.target = random.choice(jlist)
			if tre > uno and uno == cua:
				self.target = random.choice(glist)
			if cua > uno and uno == tre:
				self.target = random.choice(flist)
		...
		if player1.hp <= 0 and player2.hp > 0 and player3.hp > 0 and player4.hp > 0:
			...
			if dos < tre and dos < cua:
				self.target = player2
		
			if dos > tre and cua > tre:
				self.target = player3

			if cua < tre and cua < dos:
				self.target = player4
				
			if dos == tre == cua:
				self.target = random.choice(clist)
			if dos > tre and tre == cua:
				self.target = random.choice(jlist)
			if tre > dos and dos == cua:
				self.target = random.choice(ilist)
			if cua > dos and dos == tre:
				self.target = random.choice(hlist)
			...
			try:
				if (self.target.rect.centerx - self.rect.centerx) == 0:
					if self.target.rect.centery > self.rect.centery:
						...
						self.rect.centery += 0
				elif (target.rect.centerx - self.rect.centerx) != 0:
					x,y = direction(self, self.target)
					self.rect.centerx += self.speed*x
					self.rect.centery += self.speed*y
			except(UnboundLocalError):
...
def show_game_over_screenp4():
	...
	#draw_text1(screen, "Qop", 65, WIDTH // 2, HEIGHT // 4)
	draw_text1(screen, "Record: " + str(score) + " segundos", 80, WIDTH // 2, 250)
	draw_text1(screen, "Player 4 WINS", 20, WIDTH // 2, 400)
	draw_text1(screen, "Press Q", 20, WIDTH // 2, HEIGHT * 3/4)
...
while running:
	...
	draw_text1(screen, str(int(player2.mana))+ "/100", 10, 340, 16)

	draw_mana_bar(screen, 615, 15, player3.mana)
	draw_text1(screen, str(int(player3.mana))+ "/100", 10, 640, 16)

	draw_mana_bar(screen, 915, 15, player4.mana)
	draw_text1(screen, str(int(player4.mana))+ "/100", 10, 940, 16)
